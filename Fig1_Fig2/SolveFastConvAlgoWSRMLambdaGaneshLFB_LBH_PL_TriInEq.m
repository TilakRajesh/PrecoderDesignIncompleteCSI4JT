function [WoutRet, maxRateAchievedSOCP] = SolveFastConvAlgoWSRMLambdaGaneshLFB_LBH_PL_TriInEq(H, activeSet, PL, OptionsFlag)
    ...
    while (n <= maxIter && (n<=6 ||  (objectiveValueWSR(n-1)-objectiveValueWSR(n-6))/objectiveValueWSR(n-1)>eps))
        ...
        for ue=1:UEs
            ...
            for intUEs=1:UEs
                ...
                if (flag_PL_lambda == 1)
                    % intUEVector = [intUEVector; 2*allIntUes_PL_lambda];
%                     intUEVector = [intUEVector; sqrt(numel(allIntUes_PL_lambda))*allIntUes_PL_lambda]; % Approach 1
                    intUEVector = [intUEVector; sqrt(numel(allIntUes_PL_lambda))*allIntUes_PL_lambda]; % Approach 2
                    flag_PL_lambda = 0;
                end
            end
%             intVector= [sqrt(N0);sqrt(2)*intUEVector; ];  % Approach 1
            intVector= [sqrt(N0);intUEVector; ];  % Approach 2
